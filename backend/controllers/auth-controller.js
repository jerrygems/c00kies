...
const login = async (req, resp) => {
    try {
        ...
        if (exists) {
            ...
            if (!compResult) {
                return resp.status(500).json({ message: 'authentication failure' })
            }
...
const register = async (req, resp) => {
    try {
        ...
        if (!exists) {
            ...
            return resp.status(401).json({ message: "authentication failure" })
        }
    } catch (err) {
        console.log("error while registering user",err)
        return resp.status(500).json({ message: "something went is wrong" })
    }
}
